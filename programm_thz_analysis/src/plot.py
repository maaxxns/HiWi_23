...
def plot_FabryPerot(freq, fabry):
    plt.figure()
    plt.plot(freq, fabry.real, label='real')
    plt.plot(freq, fabry.imag, label='imag')
    plt.xlabel(r'$ \omega $')
...
def plot_Transferfunction_with_specific_n_k(freq_ref, Material):
    plt.figure()
    plt.plot(freq_ref, Transfer_function_three_slabs(freq_ref,2,2, Material, True).real, label='Transferfunction real part')
    plt.plot(freq_ref, Transfer_function_three_slabs(freq_ref,2,2,Material, True).imag, label='Transferfunction imag part')
    plt.plot(freq_ref, np.abs(Transfer_function_three_slabs(freq_ref,2,2, Material, True)), label='Transferfunction absolut')
    plt.legend()
